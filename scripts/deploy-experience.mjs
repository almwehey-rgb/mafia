...
const root=path.resolve('dist'),release=path.resolve('artifacts/experience-release');
if(process.argv[2]==='prepare'){
 ...
 await writeFile('artifacts/experience-release-manifest.json',JSON.stringify(manifest,null,2));
 ...
 if(process.argv[2]==='inspect'){console.log(JSON.stringify({id:p.id,name:p.name,rootDirectory:p.rootDirectory,framework:p.framework,outputDirectory:p.outputDirectory,buildCommand:p.buildCommand,installCommand:p.installCommand}));}
  ...
  const saved=JSON.parse(await readFile('artifacts/experience-deployment.json','utf8'));
  ...
  const manifest=JSON.parse(await readFile('artifacts/experience-release-manifest.json','utf8'));const files=[];
  ...
  await writeFile('artifacts/experience-previous-deployment.json',JSON.stringify({project,team,previousDeployment},null,2));
  const response=await fetch(`https://api.vercel.com/v13/deployments?teamId=${team}`,{method:'POST',headers,body:JSON.stringify({name:'mafia-night',project,target:'production',files,projectSettings:{framework:null,rootDirectory:null,outputDirectory:null,buildCommand:null,installCommand:null},meta:{purpose:'User explicitly requested production publication of player, host, mobile and accessibility improvements; human playtests pending',...(previousDeployment?{previousDeployment}:{})}})});
  const d=await response.json();if(!response.ok)throw Error(JSON.stringify({status:response.status,error:d.error}));
  const result={id:d.id,url:d.url,readyState:d.readyState,alias:d.alias};await writeFile('artifacts/experience-deployment.json',JSON.stringify(result,null,2));console.log(JSON.stringify(result));
 }
}
