...
نسخة المستودع مطابقة لإصدار الواجهة المنشور في 19 سبتمبر 2026، ببصمة
`ac3b2dda5011db2a`. مصدر الواجهة والخادم في `src`، ومخرجات الواجهة في `dist`.
يبني Vercel المشروع بواسطة `npm run build` وينشر مجلد `dist` وفق `vercel.json`.
لا تعتمد هذه النسخة على تحويل الطلبات إلى نشر Vercel قديم.
نشر GitHub/Vercel للواجهة لا يطبّق تغييرات Supabase تلقائيًا.
حُفظ محتوى GitHub السابق في الفرع `backup/before-latest-sync-2026-09-20`.
