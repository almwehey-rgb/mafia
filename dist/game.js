...
function schedulePreferenceSave() {
  ...
  preferenceSaveTimer = setTimeout(async () => {
    ...
    try {
      ...
      if ($('#prefsStatus')) $('#prefsStatus').textContent = discussionText('تم الحفظ ✓','Saved ✓');
    } catch {
...
function renderHostLogin() {
  ...
  $('#app').removeAttribute('data-login-shell');
  $('#app').innerHTML = `<section class="noir-entry"><div class="noir-content"><p class="noir-kicker">MAFIA · مافيا الليل</p><h2 class="noir-heading">كل وجه يخفي سرًّا</h2><div class="card hero join-card auth-card"><h1>دخول المضيف</h1><label for="hostPin">الرمز السري</label><input class="input auth-pin" id="hostPin" type="password" inputmode="numeric" maxlength="8" autocomplete="current-password" enterkeyhint="go"><button class="btn red wide" onclick="loginHost()">دخول</button></div><div class="noir-links"><button class="btn" onclick="joinForm()">دخول لاعب</button><button class="btn" onclick="spectatorForm()">دخول متفرج</button></div></div><aside class="home-emblem" aria-hidden="true"><img src="/assets/mafia-gold-icon.webp" alt="" width="220" height="220"><span>◆ MAFIA NIGHT ◆</span></aside></section>${homeCharacters()}`;
  }
  const input = $('#hostPin');
...
function joinForm(prefill = '') {
  ...
  $('#app').innerHTML = `<div class="card hero join-card"><h2>دخول الغرفة / Join room</h2>${directJoin ? `<input type="hidden" id="roomCode" value="${escapeHtml(code)}">` : `<label for="roomCode">كود الغرفة / Room code</label><input class="input" id="roomCode" inputmode="numeric" maxlength="4" autocomplete="one-time-code" dir="ltr" enterkeyhint="next" aria-describedby="joinHint"><p id="joinHint" class="entry-hint">كود من 4 أرقام لدى المضيف / Ask the host for the 4-digit code</p>`}<label for="playerName">اسمك / Your name</label><input class="input" id="playerName" maxlength="20" autocomplete="name" enterkeyhint="go" placeholder="اكتب اسمك / Enter your name"><p id="joinFeedback" class="entry-feedback" role="alert" hidden></p><button class="btn red wide" onclick="joinRoom()">دخول اللعبة / Join game</button><button class="btn wide entry-back" onclick="home()">رجوع / Back</button></div>`;
...
async function joinRoom() {
  ...
  if(!name){showJoinFeedback(discussionText('اكتب اسمك.','Enter your name.'),$('#playerName'));return;}
...
function roleReveal() {
  $('#app').innerHTML=`<section class="card hero personal-role-reveal"><div class="role-badge">${discussionText('دورك السري','Your secret role')}</div><h1 class="role-reveal-name">${escapeHtml(roleLabel(game.me.role))}</h1>${roleQuickSummary()}${personalRoleCard()}<div class="role-acknowledge"><button class="btn red wide" onclick="acknowledgeRole()" data-no-translate>${discussionText('فهمت دوري','I understand my role')}</button></div></section>`;
}
...
function renderPlayerContent() {
  ...
  if (game.phase === 'lobby') {
    ...
    $('#app').innerHTML = `<div class="card hero player-lobby"><div class="role-title">${discussionText('✅ أنت داخل الغرفة','✅ You are in the room')}</div><h2>${escapeHtml(me?.name || '')}</h2><p data-no-translate>${discussionText('بانتظار المضيف. سيظهر دورك عند بدء المباراة.','Waiting for the host. Your role appears when the game starts.')}</p><div class="counter">${game.players.length}</div><p class="muted" data-no-translate>${discussionText('لاعبون داخل الغرفة','Players in the room')}</p><div class="player-lobby-roster" aria-label="${discussionText('أسماء اللاعبين','Player names')}">${roster}</div>${me?.isHost?'<button class="btn gold" onclick="toggleDelegatedHost()">👑 فتح تحكم المضيف</button>':''}</div>`;
    return;
  }
...
function stopChatSpeech(){if('speechSynthesis' in window)window.speechSynthesis.cancel()}
function speakChatMessage(text){if(!('speechSynthesis' in window))return;stopChatSpeech();const utterance=new SpeechSynthesisUtterance(String(text));utterance.lang='ar-SA';utterance.rate=.95;window.speechSynthesis.speak(utterance)}
...
function renderChatSheet(result){
 ...
 openSheet(label,`<div class="chat-toolbar"><button class="btn" data-chat-older data-no-translate onclick="loadOlderChat()">${discussionText('رسائل أقدم','Older messages')}</button><button class="btn" type="button" onclick="stopChatSpeech()">🔇 ${discussionText('إيقاف الصوت','Stop voice')}</button></div><div class="chat-list" tabindex="0" role="region" aria-label="${discussionText('رسائل المحادثة','Chat messages')}" aria-live="polite"></div><p class="chat-status" data-no-translate role="status"></p>${game.me.muted?`<div class="status wait">${discussionText('تم كتمك بواسطة المضيف','You were muted by the host')}</div>`:`<div class="chat-compose"><input class="input" id="chatText" maxlength="240" placeholder="${discussionText('اكتب رسالة','Write a message')}"><button class="btn red" id="chatSend" onclick="sendChat()">${discussionText('إرسال','Send')}</button></div>`}`);
