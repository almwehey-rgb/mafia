...
function discussionPanel(controller) {
  ...
  if(openingDrawActive(view))return `<section class="discussion-panel draw-in-progress" data-no-translate data-controller="${controller}">${discussionRoulette(view)}<p class="draw-wait-note">${discussionText('يبدأ وقت الكلام بعد القرعة.','Speaking time starts after the draw.')}</p></section>`;
...
function bossDiscussionChoice() {
  if (game.me?.role !== 'mafia_boss' || game.me.promotedBoss || !game.me.alive || !['night','day'].includes(game.phase) || game.enabledRoles?.discussion_mode !== 'turns' || (game.discussion?.id && game.discussion.round === game.round)) return '';
  ...
  return `<section class="card" data-no-translate><h2>${discussionText('اختيارك السري للنقاش','Your private discussion choice')}</h2><p>${discussionText('تقدر تبقى متخفّي وتتكلم بدورك العادي، أو تظهر كشرطي مزيف وتشارك في قرعة البداية بفرصة مساوية لكل مشارك. اختيارك مرة واحدة ويثبت لنهاية القيم. إذا ما اخترت قبل أول نقاش، تبقى متخفّيًا.','Stay undercover and speak on your normal turn, or appear as a fake detective and join the opening draw with an equal chance for every participant. Choose once for the entire game. If you do not choose before the first discussion, you stay undercover.')}</p><div class="actions"><button class="btn ${!claim?'green':''}" aria-pressed="${!claim}" onclick="setBossDiscussionClaim(false)">${discussionText('أبقى متخفّي','Stay undercover')}</button><button class="btn ${claim?'green':''}" aria-pressed="${claim}" onclick="setBossDiscussionClaim(true)">${discussionText('أظهر كشرطي مزيف','Join as a fake detective')}</button></div></section>`;
}
