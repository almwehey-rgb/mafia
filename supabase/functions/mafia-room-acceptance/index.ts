...
async function routeMessages(context:RoomRouteContext) {
  let {body, action, ip, now, started, code, room, players, me, host, authenticatedSpectator}=context;
  {
      ...
      if (action === "sendMessage") {
        ...
        const { error: insertError } = await db.from("mafia_messages").insert({ room_code: code, round: room.round, channel, author_id: me.id, author_name: channel === "jail" && me.role !== "jailer" ? "Ø§Ù„Ø³Ø¬ÙŠÙ†" : me.name, content });
      ...
      const page=(data||[]).slice(0,80);
      return out({ channel, hasMore:(data||[]).length>80, nextCursor:page.length?String(page.at(-1).id):null, messages: page.reverse().map((message) => channel === "jail" ? { ...message, author_id: undefined, author_name: message.author_id === room.jailed_player ? "Ø§Ù„Ø³Ø¬ÙŠÙ†" : "Ø§Ù„Ø³Ø¬Ù‘Ø§Ù†" } : message) });
  ...
  let {body, action, ip, now, started, code, room, players, me, host, authenticatedSpectator}=context;
  {
      ...
      for (const detective of players.filter((p) => p.alive && p.role === "detective")) {
        ...
        if (canAct(detective)) for (const id of selected(detective).slice(0, detectiveLimit(room, players))) {
          ...
          if (target) history.push({ round: room.round, name: target.name, result: target.role === "mafia" || (target.role === "mafia_boss" && !enabledRoles(room.enabled_roles).godfather_innocent) ? "MAFIA" : "INNOCENT" });
        }
...
async function handleRequest(request:Request) {
  ...
  try {
    ...
    if(Date.now()>1789386490895)return out({error:'ACCEPTANCE_EXPIRED'},403);
    if(action==='hostLogin'){
      const loginBucket=loginBuckets.get(ip);
      if(!loginBucket||loginBucket.reset<now)loginBuckets.set(ip,{count:1,reset:now+900000});
      else if(++loginBucket.count>5)return out({error:'LOGIN_RATE_LIMITED'},429);
      if(await sha256(String(body.pin))!=="ae1d807d44bef64698d26faf13138d9f89942782f5e8a60bee7b2457f4aec273")return out({error:'INVALID_PIN'},403);
      loginBuckets.delete(ip);return out({hostAccessToken:"7ae000e6d768d0bf0c0bf6ac71adfbbc7564185ace4682e28f67608c956805c1",preferences:{mafiaCount:1,detectiveCount:1,doctor:false,jailer:false,lawyer:false,full_trial:false,discussion_mode:'turns',speaker_seconds:15}});
    }
    if(action==='create'){
      if(body.hostAccessToken!=="7ae000e6d768d0bf0c0bf6ac71adfbbc7564185ace4682e28f67608c956805c1")return out({error:'UNAUTHORIZED'},403);
      const {room,players}=await load('9681');
    ...
    if(action==='hostPreferences')return body.hostAccessToken==="7ae000e6d768d0bf0c0bf6ac71adfbbc7564185ace4682e28f67608c956805c1"?out({preferences:cleanPreferences(body.settings||{}),saved:true}):out({error:'UNAUTHORIZED'},403);
    ...
    if(action!=='health'&&String(body.code)!=='9681')return out({error:'ROOM_NOT_FOUND'},404);
