...
function roleLabel(role){
  if(!kidsModeActive())return roleNames[role]||role;
...
const eventText = {
  ...
  nomination_tie: '🤝 تعادل الترشيح — لا متهم. تبدأ ليلة جديدة بدون نقاش نهار.',
...
function eventCards() {
  ...
  return events.map((event) => { const startRound = normalizeEnabledRoles(game.enabledRoles || {}).mafia_kill_start_round || 2; const delayed = event === 'mafia_delayed' ? discussionText(`🌙 لا يوجد اغتيال مافيا هذه الليلة. يبدأ من الليلة ${startRound}.`,`🌙 No Mafia kill tonight. It begins on night ${startRound}.`) : null; return `<div class="event">${delayed || (game.enabledRoles?.kids_mode&&(kids[event])?kids[event]:(eventText[event] || escapeHtml(event)))}</div>`; }).join('');
}
...
function setupRoleCards(roles, review = false) {
  const cards = enabledRoles.kids_mode
    ...
    : [
      ['mafia_boss', '👑', 'زعيم المافيا', Math.min(1, roles.mafia), true],
      ['mafia', '◆', 'فرد المافيا', Math.max(0, roles.mafia - 1), true],
...
function renderHostContent() {
  ...
  if (game.phase === 'day') {
    ...
    $('#app').innerHTML = `${phaseBar()}<div class="grid"><div class="card hero"><div class="role-title">☀️ الصباح</div>${eventCards()}${deaths || '<div class="status">لم يمت أحد هذه الليلة</div>'}${discussionPanel(true)}<div class="status ${game.lawyerReady ? '' : 'wait'}">${game.lawyerReady ? 'المحامي جاهز ✅' : 'بانتظار حماية المحامي…'}</div><div class="status ${game.jailerReady ? '' : 'wait'}">${game.jailerReady ? 'السجّان اختار السجين ✅' : 'بانتظار اختيار السجّان…'}</div><button class="btn gold wide" data-discussion-vote ${ready ? '' : 'disabled'} onclick="hostAction('startVote')">بدء التصويت</button></div><div class="card"><h2>الأحياء</h2><div class="players">${playerList(false, false, alivePlayers())}</div></div></div>`;
    return;
  }
...
async function startGame() {
  ...
  try {
    ...
    renderHost();
  } catch (error) { notice(error.code === 'STALE_GAME' ? 'تغيّرت حالة الغرفة. انتظر تحديث الشاشة ثم حاول مجددًا.' : 'تأكد من وجود لاعبين كافين / Check player count'); }
  finally { lifecycleRequestPending = false; }
}
...
function roleReveal() {
  $('#app').innerHTML=`<section class="card hero personal-role-reveal"><div class="role-badge">دورك السري / Your secret role</div><p class="muted">اضغط صورة الكرت لتقلبها وتقرأ شرح دورك.</p>${personalRoleCard()}<button class="btn red wide" onclick="acknowledgeRole()">فهمت دوري / I understand</button></section>`;
}
...
function renderPlayerContent() {
  ...
  const me = game.me;
  if (game.phase === 'lobby') {
    $('#app').innerHTML = `<div class="card hero"><div class="role-title">✅ دخلت الغرفة</div><h2>${escapeHtml(me?.name || '')}</h2><p>انتظر المضيف لبدء القيم. انقطاع الاتصال المؤقت يحفظ مقعدك.</p><button class="btn" onclick="leaveRoom()">🚪 مغادرة الغرفة</button><div class="counter">${game.players.length}</div><p class="muted">لاعبين داخل الغرفة / Players joined</p>${me?.isHost?'<button class="btn gold" onclick="toggleDelegatedHost()">👑 فتح تحكم المضيف</button>':''}</div>`;
    return;
  }
...
function renderNight() {
  ...
  if (me.role === 'serial_killer') {
    ...
    $('#app').innerHTML = `<div class="card"><div class="role-title">${roleNames.serial_killer}</div>${me.acted ? '<h2 class="ok">تم اختيار الضحية ✅</h2>' : `<h2>من ستقتل الليلة؟</h2>${choiceButtons(targets, 'nightAction', { icon: '🩸' })}`}</div>`;
    return;
  }
  if (me.role === 'cupid') {
    if (game.round !== 2) {
...
function enhanceJourney(controller){
  ...
  if(hint){const node=document.createElement('p');node.className='journey-hint';node.setAttribute('data-no-translate','');node.textContent=hint;document.querySelector('#app .phase-bar').insertAdjacentElement('afterend',node);}
...
/* e14ux8 compatibility patch: direct Mafia Boss handover during role reveal + clearer vote selection */
async function resignMafiaLeader(successor){
  if(!successor)return;
  const target=(game.me?.mafiaTeam||[]).find(function(p){return p.id===successor;});
  if(!target)return;
  if(!confirm(discussionText('تسليم قيادة المافيا إلى ','Transfer Mafia leadership to ')+target.name+'؟'))return;
  try{
    game=await api({action:'electMafiaLeader',code:game.code,id:playerId,playerToken,target:'RESIGN',successor:successor});
    renderPlayer();
  }catch{
    notice(discussionText('تعذر تسليم القيادة.','Could not transfer leadership.'));
  }
}
function leaderElectionCard(){
  ...
  if(game.me.leaderElection){
    const ended=Date.now()+discussionClockOffset>=game.me.leaderDeadline;
    return '<section id="leaderElectionCard" class="card" data-no-translate>'
      +'<h2>'+discussionText('اختيار زعيم المافيا الجديد','Choose the new Mafia Boss')+'</h2>'
      +'<p>'+discussionText('تصويت سري بعد التنازل. الأكثر أصواتًا يصبح الزعيم، والتعادل بقرعة.','Secret vote after resignation. Highest votes wins; ties are randomized.')+'</p>'
      +(ended
        ? '<button class="btn gold" onclick="electMafiaLeader(&quot;FINALIZE&quot;)">'+discussionText('حسم التصويت وتحديث الكروت','Finalize vote and refresh roles')+'</button>'
        : game.me.leaderVote
          ? '<p>'+discussionText('تم تسجيل صوتك. بانتظار بقية الفريق.','Your vote is saved. Waiting for the team.')+'</p>'
          : choiceButtons((game.me.mafiaTeam||[]).filter(function(p){return p.id!==game.me.leaderFormer&&p.alive!==false;}),'electMafiaLeader',{icon:'👑'}))
      +'</section>';
  }
  if(game.me.role!=='mafia_boss')return '';
  const successors=(game.me.mafiaTeam||[]).filter(function(p){
    return p.id!==game.me.id && game.players.some(function(x){return x.id===p.id&&x.alive;});
  });
  if(!successors.length)return '';
  return '<section id="leaderElectionCard" class="card" data-no-translate>'
    +'<h3>'+discussionText('👑 تسليم قيادة المافيا','👑 Transfer Mafia leadership')+'</h3>'
    +'<p>'+discussionText('اختر فرد المافيا الذي يستلم الرئاسة مباشرة قبل تأكيد دورك.','Choose the Mafia member who becomes Boss immediately before you confirm your role.')+'</p>'
    +successors.map(function(p){
      return '<button class="btn red wide" onclick="resignMafiaLeader('+jsArg(p.id)+')">👑 '+escapeHtml(p.name)+'</button>';
    }).join('')
    +'</section>';
}
function choiceButtons(players, action, options = {}) {
  const inner = players.map(function(player){
    const selected = action==='castVote' && game?.me?.voteTarget===player.id;
    return '<button class="pick'+(selected?' selected':'')+'" data-target="'+escapeHtml(player.id)+'" aria-pressed="'+(selected?'true':'false')+'" onclick="'+action+'('+jsArg(player.id)+')">'
      +(options.icon || '👤')+' '+escapeHtml(player.name)
      +(player.id===playerId?' ('+discussionText('أنت','You')+')':'')
      +(selected?' ✓':'')
      +'</button>';
  }).join('');
  return '<div class="choice-stack">'+inner+'</div>';
