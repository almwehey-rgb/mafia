...
try {
 ...
 const report=[];
 for(const lang of ['ar','en'])for(const theme of ['dark','light'])for(const width of [320,390,768,1280])for(const view of views){
 await page.setViewportSize({width,height:900});await page.evaluate(({lang,theme})=>{localStorage.setItem('mafia-lang',lang);document.documentElement.lang=lang;document.documentElement.dir=lang==='ar'?'rtl':'ltr';document.body.classList.toggle('light',theme==='light')},{lang,theme});
 try{
 ...
 report.push({view,width,lang,theme,...overflow});
 if(overflow.scroll>width)await page.screenshot({path:'artifacts/audit-'+view+'-'+width+'-'+lang+'-'+theme+'.png',fullPage:true});
 }catch(e){report.push({view,width,error:e.message});}
 }
 console.log(JSON.stringify(report.filter(r=>r.error||r.scroll>r.width),null,2));console.log('Browser errors:',errors);await (await import('node:fs/promises')).writeFile('artifacts/layout-audit.json',JSON.stringify(report,null,2));
 assert.deepEqual(report.filter(r=>r.error||r.scroll>r.width),[], 'Every view must render without horizontal overflow');
 assert.deepEqual(errors,[], 'No browser runtime errors');console.log('PASS '+report.length+' layout screen variants');
