...
try {
 ...
 const views=['host-day','player-day','player-night','host-trial'];
 const report=[];
 for(const width of [320,390])for(const view of views){
 ...
 console.log(JSON.stringify(report.filter(r=>r.error||r.scroll>r.width),null,2));console.log('Browser errors:',errors);await (await import('node:fs/promises')).writeFile('artifacts/layout-small-audit.json',JSON.stringify(report,null,2));
