...
test('Chat receiver reads without sending; latest 80 messages and channel isolation',async()=>{
 ...
 publicFixture.players.find(p=>p.id==='bot').name='بوت بدر';
 const publicSend=await publicFixture.call('sendMessage',{id:'a',playerToken:'token-a',text:'أشك في بوت بدر'});
 assert.equal(publicSend.status,200);
 assert.equal(publicFixture.messages.some(m=>m.author_id==='bot'&&m.content.includes('بوت بدر')),true);
